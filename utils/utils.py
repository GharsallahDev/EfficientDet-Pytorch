...
def plot_one_box(img, coord, label=None, score=None, color=None, line_thickness=None):
    ...
    if label:
        tf = max(tl - 2, 1)
        text = f"{label}: {score:.0%}" if score is not None else label
        t_size = cv2.getTextSize(text, 0, fontScale=float(tl) / 3, thickness=tf)[0]
        c2 = c1[0] + t_size[0] + 3, c1[1] - t_size[1] - 3
        cv2.rectangle(img, c1, c2, color, -1)
        cv2.putText(img, text, (c1[0], c1[1] - 2), 0, float(tl) / 3, [225, 255, 255], thickness=tf, lineType=cv2.FONT_HERSHEY_SIMPLEX)
